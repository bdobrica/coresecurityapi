...
foreach ($diplomas as $diploma) {
	...
	$pdf->MultiCell (0, $h, $pdf->fix('Prin prezenta se adeverește că doamna '.$person->get('last_name').' '.$person->get('first_name').' având CNP '.$person->get('uin').', a absolvit cursul de '.$product->get('current anc name').', cod COR '.$product->get('current corno').', în perioada '.wp_crm_date($cnfpa_interval).', urmând a i se elibera un certificat de absolvire.'), 0, 1);
	$pdf->SetY ($pdf->GetY() + $h);
