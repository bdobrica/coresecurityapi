...
$pdf->SetY ($pdf->GetY() + $h);
$pdf->MultiCell (0, $h, $pdf->fix($company->get('name').' este companie autorizată de Ministerul Muncii, Familiei și Egalității de Șanse și Ministerul Educației, Cercetării, Tineretului și Sportului să organizeze programul de formare profesională pentru ocupația de '.$product->get('current anc name').' prin autorizația '.$product->get('current anc auth').'.'));
$pdf->SetY ($pdf->GetY() + 2*$h);
