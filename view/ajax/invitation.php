...
if ($wp_crm_user->get ('defaults', 'promoter_invitations') < 1)
	_die ('Imi pare rau! Ai depasit numarul maxim de invitatii pe care le poti trimite.');
...
if ($_POST['object']) {
	...
	if (!$action['error']) {
		...
		echo json_encode (array (array (
			'key' => 'invitations',
			'val' => $wp_crm_user->get ('defaults', 'promoter_invitations')
			)));
		exit (0);
		}
	}
