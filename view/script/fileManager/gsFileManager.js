...
function gs_show_loading() {
	$("#gs_dir_content").html('<div class="loadingDiv">&nbsp;</div>');
}
...
function gsGetSelectedItems(){
	var arr = new Array();
	$("#gs_content_table div.rowSelected").each(function(){
...
gs_filemanager_languages['en'][1] = 'Directorul Curent';
gs_filemanager_languages['en'][2] = 'Clipboard';
gs_filemanager_languages['en'][3] = 'Upload';
gs_filemanager_languages['en'][4] = 'Fisier nou';
gs_filemanager_languages['en'][5] = 'Folder nou';
gs_filemanager_languages['en'][6] = 'Paste';
gs_filemanager_languages['en'][7] = 'Nume';
gs_filemanager_languages['en'][8] = 'Tip';
gs_filemanager_languages['en'][9] = 'Dimensiune';
gs_filemanager_languages['en'][10] = 'Ultima modificare';
gs_filemanager_languages['en'][11] = 'Imbunatateste';
gs_filemanager_languages['en'][12] = 'Notepad';
gs_filemanager_languages['en'][13] = 'ImageViewer';
gs_filemanager_languages['en'][14] = 'Copiaza';
gs_filemanager_languages['en'][15] = 'Cut';
gs_filemanager_languages['en'][16] = 'Redenumeste';
gs_filemanager_languages['en'][17] = 'Copiaza ca';
gs_filemanager_languages['en'][18] = 'Descarca';
gs_filemanager_languages['en'][19] = 'Sterge';
gs_filemanager_languages['en'][20] = 'Deschide';
gs_filemanager_languages['en'][21] = 'CKeditor';
gs_filemanager_languages['en'][22] = 'JCrop';
gs_filemanager_languages['en'][23] = 'Selectare tot';
gs_filemanager_languages['en'][24] = 'Deselectare tot';
...
gs_filemanager_languages['en'][40] = 'Arhiveaza zip';
gs_filemanager_languages['en'][41] = 'Zip archive name';
gs_filemanager_languages['en'][42] = 'Dezarhiveaza zip';
...
gs_filemanager_languages['en'][48] = 'Scanare document';
...
if (jQuery) (function(jQuery){
	...
	jQuery.extend(jQuery.fn, {
		gsFileManager: function(o) {
            ...
            menuHtml += '<a id="gs_pastebutton" class=\'gs_dir_content_button\'>&nbsp;' + gs_getTranslation(o.language, 48)+ '&nbsp;</a>';
            var wrapperHtml = '<div id=\'gs_dir_list\' class=\'gs_dir_list\' onClick="$(this).doGSAction({action: 21})"></div>';
			wrapperHtml    += '<div class=\'gs_dir_content\' onClick="$(this).doGSAction({action: 21})">' 
			...
			$('#gs_uploadAddField').click(function (e){
				...
				uploadsFieldsHolder.append('<div><a onClick="$(this).parent().remove();" style="font-size: 12px" class=\'gs_dir_content_button\'>&nbsp;' + gs_getTranslation(o.language, 46)+ '&nbsp;</a> <input type="file" name="filename_' + uploadsFieldsLength + '" size="30"/></div>');
			});
			
			$('#gs_jcrop_form').ajaxForm({
			    beforeSubmit: function () {
			    	$('#gsimagecropzoom').append('<div class="loadingDiv">&nbsp;</div>');
				}, 
			...
			$('#gsUploadForm').ajaxForm({
				    beforeSubmit: function () {
				    	$('#gsuploadfiles').append('<div class="loadingDiv">&nbsp;</div>');
					}, 
					success: function (responseText, statusText, xhr, $form) {
						gsCheckResponce(responseText);
						$('#'+$("#curDir").attr('rel')).trigger('click');
						$('#gsuploadfiles').find('div.loadingDiv').remove(); 
					},
					dataType: 'script'
			});
			...
			function showDirs (gsfiless) {
				...
				if (gs_lastparent.length > 0) {
				    fileshtml += "<tr><td><div class='directory directory_info gsItem' rel=\'up\'><a href='javascript:void(0)' ondblclick=\"$('#" + $("#curDir").attr('rel')+ "').parent().parent().parent().children('a').trigger('click'); return false\"> ..up</a></div></td><td>Dir</td></tr>";
				}
				if (gsfiless.length > 0) {
					for (var numf in gsfiless) {
						var curItem = gsfiless[numf];
						gs_cur_items[curItem.id] = curItem;
						fileshtml += "<tr><td><div class='directory directory_info gsItem' rel=\'" + curItem.id + "\'><a href='javascript:void(0)' ondblclick=\"$('#"+curItem.id+"').trigger('click'); return false\">" + curItem.name + "</a></div></td><td>Dir</td><td>0</td><td>"+curItem.getLastMod()+"</td></tr>";
					}
				}
                return fileshtml;
			}
			...
			function showContent (gsdirss, gsfiless) {
				...
				$('#gs_dir_content').html(tableheader + dirshtml + fileshtml + "</table>");
				
				$('div.file').contextMenu({
					menu: 'gsFileMenu'
				},
					function(action, el, pos) {
				...
				$('table.dirs_files_table tr').find('div.gsItem').bind('click', function(e){
			...
			function showTree(c, t) {
				...
				jQuery.ajax({
					...
					success: function(data) {
						...
						$("#curDir").html(decodeURIComponent(t));
			...
			function setHandlers(t) {
				...
				$(t).find('LI > A').bind('click', function () {
					...
					$(this).parent().find(' > span').removeClass('toggleplus').addClass('toggleminus');
				});
				$(t).find('LI > SPAN').bind('click', function () {
					var thisEl = $(this);
			...
			function showRoot(){
				showTree( $('#gs_dir_list'), escape(o.root));
				    ...
				    buttons: [{ 
					           ...
					           click: function() {
								   ...
								   sendAndRefresh(o, dataForSend, true, function(data) {
										...
										$('#gsimageviewer_content').html('<img src="'+ gs_makeUrl(o.script, jQuery.param(dataForSend) +'&time='+ new Date().getTime())+'" id="gs_imageviewer_image"/>');  
								   });
					       }
				     }]
				});
