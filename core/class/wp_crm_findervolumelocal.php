...
/**
 * WP_CRM_Finder driver for local filesystem.
 *
 * @author Dmitry (dio) Levashov
 * @author Troex Nevelin
 * @author Bogdan Dobrica
 **/
class WP_CRM_FinderVolumeLocal extends WP_CRM_FinderVolumeDriver {
	...
	protected function _extract($path, $arc) {
		...
		if ($this->quarantine) {
			...
			if ($symlinks) {
				return $this->setError(WP_CRM_Finder::ERROR_ARC_SYMLINKS);
			}

			// check max files size
			if ($this->options['maxArcFilesSize'] > 0 && $this->options['maxArcFilesSize'] < $this->archiveSize) {
				return $this->setError(WP_CRM_Finder::ERROR_ARC_MAXSIZE);
			}
