...
foreach (($wp_crm_invoices->get()) as $wp_crm_invoice) {
	...
	if (isset($monthly['number'][date('N.D', $date)])) {
		$monthly['number'][date('N.D', $date)] ++;
		$monthly['paying'][date('N.D', $date)] += $wp_crm_invoice->is('paid') ? 1 : 0;
		}
	else {
		$monthly['number'][date('N.D', $date)] = 1;
		$monthly['paying'][date('N.D', $date)] = $wp_crm_invoice->is('paid') ? 1 : 0;
		}
	}
...
$title = new OFC_Elements_Title( 'Daily ('.$interval.')' );
