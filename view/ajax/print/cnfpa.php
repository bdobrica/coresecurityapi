...
//	$person->set ('diploma', $product->get('current code'));
	...
	$pdf->SetXY (20 + $olx, 185 + $oly);
	$pdf->Cell (12, 5, $diploma, 0, 0, 'C');
	$pdf->SetXY (57 + $olx, 185 + $oly);
	...
	$ancname = $product->get ('current ancname');
	$width = $pdf->GetStringWidth ($ancname);
	$fontoff = 1;

	while (($width = $pdf->GetStringWidth ($ancname)) > 100) $pdf->style ('cnfpa:-'.($fontoff++));
