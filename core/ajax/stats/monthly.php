...
foreach (($wp_crm_invoices->get()) as $wp_crm_invoice) {
	...
	if (isset($monthly['number'][date('m.M', $date)])) {
		$monthly['number'][date('m.M', $date)] ++;
		$monthly['paying'][date('m.M', $date)] += $wp_crm_invoice->is('paid') ? 1 : 0;
		}
	else {
		$monthly['number'][date('m.M', $date)] = 1;
		$monthly['paying'][date('m.M', $date)] = $wp_crm_invoice->is('paid') ? 1 : 0;
		}
	}
...
$title = new OFC_Elements_Title( 'Monthly ('.$interval.')' );
