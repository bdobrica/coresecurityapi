<?php
/**
 * Base class for WP_CRM_Finder volume.
 * Provide 2 layers:
 ...
 **/
abstract class WP_CRM_FinderVolumeDriver {
	...
	public function mount(array $opts) {
		...
		$this->id = $this->driverId.(!empty($this->options['id']) ? $this->options['id'] : WP_CRM_Finder::$volumesCnt++).'_';
	...
	public function file($hash) {
		...
		return ($file = $this->stat($path)) ? $file : $this->setError(WP_CRM_Finder::ERROR_FILE_NOT_FOUND);
	...
	public function dir($hash, $resolveLink=false) {
		if (($dir = $this->file($hash)) == false) {
			return $this->setError(WP_CRM_Finder::ERROR_DIR_NOT_FOUND);
		}
	...
	public function scandir($hash) {
		...
		return $dir['read']
			...
			: $this->setError(WP_CRM_Finder::ERROR_PERM_DENIED);
	}
	...
	public function mkdir($dst, $name) {
		...
		if (!$this->nameAccepted($name)) {
			return $this->setError(WP_CRM_Finder::ERROR_INVALID_NAME);
		}
		
		if (($dir = $this->dir($dst)) == false) {
			return $this->setError(WP_CRM_Finder::ERROR_TRGDIR_NOT_FOUND, '#'.$dst);
		}
		...
		$stat = $this->stat($dst);
		if (!empty($stat)) {
			return $this->setError(WP_CRM_Finder::ERROR_EXISTS, $name);
		}
		$this->clearcache();
	...
	public function rename($hash, $name) {
		...
		if (!$this->nameAccepted($name)) {
			return $this->setError(WP_CRM_Finder::ERROR_INVALID_NAME, $name);
		}
		...
		if (!empty($file['locked'])) {
			return $this->setError(WP_CRM_Finder::ERROR_LOCKED, $file['name']);
		}
	...
	public function duplicate($hash, $suffix='copy') {
		if ($this->commandDisabled('duplicate')) {
			return $this->setError(WP_CRM_Finder::ERROR_COPY, '#'.$hash, WP_CRM_Finder::ERROR_PERM_DENIED);
		}
		
		if (($file = $this->file($hash)) == false) {
			return $this->setError(WP_CRM_Finder::ERROR_COPY, WP_CRM_Finder::ERROR_FILE_NOT_FOUND);
		}
	...
	public function upload($fp, $dst, $name, $tmpname) {
		...
		if ($mime == 'unknown' && $this->mimeDetect == 'internal') {
			$mime = WP_CRM_FinderVolumeDriver::mimetypeInternalDetect($name);
		}
		...
		if (!$upload) {
			return $this->setError(WP_CRM_Finder::ERROR_UPLOAD_FILE_MIME);
		}

		if ($this->uploadMaxSize > 0 && filesize($tmpname) > $this->uploadMaxSize) {
			return $this->setError(WP_CRM_Finder::ERROR_UPLOAD_FILE_SIZE);
		}
		...
		if ($file) { // file exists
			if ($this->options['uploadOverwrite']) {
				if (!$file['write']) {
					...
					return $this->setError(WP_CRM_Finder::ERROR_NOT_REPLACE, $name);
				} 
				$this->remove($file);
			} else {
	...
	public function paste($volume, $src, $dst, $rmSrc = false) {
		$err = $rmSrc ? WP_CRM_Finder::ERROR_MOVE : WP_CRM_Finder::ERROR_COPY;
		
		if ($this->commandDisabled('paste')) {
			return $this->setError($err, '#'.$src, WP_CRM_Finder::ERROR_PERM_DENIED);
		}

		if (($file = $volume->file($src, $rmSrc)) == false) {
			return $this->setError($err, '#'.$src, WP_CRM_Finder::ERROR_FILE_NOT_FOUND);
		}
		...
		if (($dir = $this->dir($dst)) == false) {
			return $this->setError($err, $errpath, WP_CRM_Finder::ERROR_TRGDIR_NOT_FOUND, '#'.$dst);
		}
		
		if (!$dir['write'] || !$file['read']) {
			return $this->setError($err, $errpath, WP_CRM_Finder::ERROR_PERM_DENIED);
		}
		...
		if (($test = $volume->closest($src, $rmSrc ? 'locked' : 'read', $rmSrc))) {
			return $rmSrc
				? $this->setError($err, $errpath, WP_CRM_Finder::ERROR_LOCKED, $volume->path($test))
				: $this->setError($err, $errpath, WP_CRM_Finder::ERROR_PERM_DENIED);
		}
		...
		if ($stat) {
			if ($this->options['copyOverwrite']) {
				...
				if (!$this->isSameType($file['mime'], $stat['mime'])) {
					return $this->setError(WP_CRM_Finder::ERROR_NOT_REPLACE, $this->_path($test));
				}
				...
				if (($locked = $this->closestByAttr($test, 'locked', true))) {
					return $this->setError(WP_CRM_Finder::ERROR_LOCKED, $this->_path($locked));
				}
				// remove existed file
				if (!$this->remove($test)) {
					return $this->setError(WP_CRM_Finder::ERROR_REPLACE, $this->_path($test));
				}
			} else {
		...
		if ($volume == $this) {
			...
			if ($this->_inpath($destination, $source)) {
				return $this->setError(WP_CRM_Finder::ERROR_COPY_INTO_ITSELF, $path);
			}
		...
		if (!$this->options['copyTo'] || !$volume->copyFromAllowed()) {
			return $this->setError(WP_CRM_Finder::ERROR_COPY, $errpath, WP_CRM_Finder::ERROR_PERM_DENIED);
		}
		...
		if ($rmSrc) {
			if ($volume->rm($src)) {
				...
				return $this->setError(WP_CRM_Finder::ERROR_MOVE, $errpath, WP_CRM_Finder::ERROR_RM_SRC);
			}
		}
		return $this->stat($path);
	}
	...
	public function getContents($hash) {
		...
		if ($file['mime'] == 'directory') {
			return $this->setError(WP_CRM_Finder::ERROR_NOT_FILE);
		}
	...
	public function extract($hash) {
		...
		if (!$archiver) {
			return $this->setError(WP_CRM_Finder::ERROR_NOT_ARCHIVE);
		}
	...
	public function archive($hashes, $mime) {
		...
		if (!$archiver) {
			return $this->setError(WP_CRM_Finder::ERROR_ARCHIVE_TYPE);
		}
		...
		foreach ($hashes as $hash) {
			if (($file = $this->file($hash)) == false) {
				return $this->error(WP_CRM_Finder::ERROR_FILE_NOT_FOUND, '#'+$hash);
			}
			if (!$file['read']) {
				return $this->error(WP_CRM_Finder::ERROR_PERM_DENIED);
			}
			$path = $this->decode($hash);
			if (!isset($dir)) {
				$dir = $this->_dirname($path);
	...
	public function rm($hash) {
		return $this->commandDisabled('rm')
			? array(WP_CRM_Finder::ERROR_ACCESS_DENIED)
	...
	protected function mimetype($path) {
		...
		if ($this->mimeDetect == 'finfo') {
			...
			$type = WP_CRM_FinderVolumeDriver::mimetypeInternalDetect($path);
		}
		...
		return $type == 'unknown' && $this->mimeDetect != 'internal'
			? WP_CRM_FinderVolumeDriver::mimetypeInternalDetect($path)
			: $type;
	...
	static protected function mimetypeInternalDetect($path) {
		...
		return isset(WP_CRM_FinderVolumeDriver::$mimetypes[$ext]) ? WP_CRM_FinderVolumeDriver::$mimetypes[$ext] : 'unknown';
	...
	protected function copy($src, $dst, $name) {
		...
		if (!empty($srcStat['thash'])) {
			...
			return $stat && $this->_symlink($target, $dst, $name)
				...
				: $this->setError(WP_CRM_Finder::ERROR_COPY, $this->_path($src));
		} 
		...
		if ($srcStat['mime'] == 'directory') {
			...
			if (($test && $test['mime'] != 'directory') || !$this->_mkdir($dst, $name)) {
				return $this->setError(WP_CRM_Finder::ERROR_COPY, $this->_path($src));
			}
	...
	protected function move($src, $dst, $name) {
		...
		return $this->setError(WP_CRM_Finder::ERROR_MOVE, $this->_path($src));
	}
	...
	protected function copyFrom($volume, $src, $destination, $name) {
		...
		if (($source = $volume->file($src)) == false) {
			return $this->setError(WP_CRM_Finder::ERROR_COPY, '#'.$src, $volume->error());
		}
		
		$errpath = $volume->path($src);
		
		if (!$this->nameAccepted($source['name'])) {
			return $this->setError(WP_CRM_Finder::ERROR_COPY, $errpath, WP_CRM_Finder::ERROR_INVALID_NAME);
		}
		...
		if ($source['mime'] == 'directory') {
			...
			if ((!$stat || $stat['mime'] != 'directory') && !$this->_mkdir($destination, $name)) {
				return $this->setError(WP_CRM_Finder::ERROR_COPY, $errpath);
			}
	...
	protected function remove($path, $force = false) {
		...
		if (empty($stat)) {
			return $this->setError(WP_CRM_Finder::ERROR_RM, $this->_path($path), WP_CRM_Finder::ERROR_FILE_NOT_FOUND);
		}
		
		if (!$force && !empty($stat['locked'])) {
			return $this->setError(WP_CRM_Finder::ERROR_LOCKED, $this->_path($path));
		}
		...
		if ($stat['mime'] == 'directory') {
			...
			if (!$this->_rmdir($path)) {
				return $this->setError(WP_CRM_Finder::ERROR_RM, $this->_path($path));
			}
