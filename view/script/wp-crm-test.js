...
var $wpcrmui = new function () {
	...
	this.scan = function (w) {
		...
		jQuery('.wp-crm-view-nodeadd', w).click(function(e){
			e.preventDefault ();
			var u = '/wp-content/themes/wp-crm/ajax/nodeadd.php';
			$wpcrmui.wo (e, u);
			});
		jQuery('.wp-crm-view-nodeedit', w).click(function(e){
			e.preventDefault ();
			jQuery('.wp-crm-view-tree-menu').hide();
			var u = '/wp-content/themes/wp-crm/ajax/nodeedit.php';
			$wpcrmui.wo (e, u);
			});
		...
		if (w) {
			...
			jQuery('input[type="submit"]', w).click(function(f){
				...
				var p = jQuery(this).closest('form').serialize() + '&object=' + ($wpcrmui.obj.length > 0 ? $wpcrmui.obj[$wpcrmui.obj.length-1] : '') + '&' + jQuery(this).attr('name')+'=1';
				if ($wpcrmui.url.length > 0)
		...
		var rnd = {
			...
			redraw: function () {
				...
				pS.eachNode(function (n, p) {
					var w = 10;
					ctx.fillStyle = n.linkto ? 'green' : 'red';
					ctx.fillRect (p.x - w/2, p.y - w/2, w, w);
					ctx.font = '12px Calibri';
					ctx.fillText (n.data, p.x + w, p.y);
					});
				},
			initMouse: function() {
				var d = null;
					...
					clicked: function (e) {
						...
						if (d && d.node !== null) {
							...
							if ($wpcrmui.tod && $wpcrmui.tod.linkto) {
								$wpcrmui.par.addEdge ($wpcrmui.tod, d.node);
	...
	this.leaf = function (l) {
		...
		jQuery.each(l.leaves, function(i,v) {
			...
			$wpcrmui.par.addEdge(l.id, v.id);
			$wpcrmui.leaf (v);
			});
		}
	this.tree = function (c, t) {
		this.par = arbor.ParticleSystem (0, 2000, 0.5);
	...
	this.wo = function (e, u, v) {
		...
		jQuery.ajax({
			...
			success: function(d){
				...
				if ($wpcrmui.txt.last().html()) {
					var txt = $wpcrmui.txt.last().clone();
					$wpcrmui.txt.last().after(txt).hide();
					$wpcrmui.txt.add(txt);
					txt.empty().html(d);
					}
				else $wpcrmui.txt.last().html(d);
...
//			jQuery('html, body').animate({'scrollTop': 0});
			this.sha.css({'opacity': 0, 'display': 'block', 'height': jQuery(document).height()}).animate({'opacity': .8}, function(){
				var txt = $wpcrmui.txt.last();

				$wpcrmui.win.modal('show');
				$wpcrmui.win.css({'height':'auto'});

				/* flat-ui element */
				jQuery('.nav-tabs a', txt).on('click', function(e){e.preventDefault();jQuery(this).tab('show')});
				jQuery('.select-sm', txt).selectpicker({size: false, style: 'btn-sm btn-wide btn-primary', menuStyle: 'dropdown-inverse' });
				jQuery('[data-toggle="radio"]', txt).radio();
				jQuery('[data-toggle="checkbox"]', txt).checkbox();


				/* ui elements */
				jQuery('.wp-crm-form-seller', txt).seller({title:'Emitent:',url:'/wp-content/themes/wp-crm/ajax/widget/seller.php'});
				jQuery('.wp-crm-form-buyer', txt).buyer({title:'Cumparator',url:'/wp-content/themes/wp-crm/ajax/widget/buyer.php'});
				jQuery('.wp-crm-form-person', txt).person({title:'Persoana',url:'/wp-content/themes/wp-crm/ajax/widget/person.php'});
				jQuery('.wp-crm-form-product', txt).product({title:'Produse',url:'/wp-content/themes/wp-crm/ajax/widget/product.php'});

				jQuery('.wp-crm-form-tab-add', txt).click(function(e){
				...
				jQuery('.wp-crm-form-date', txt).datepicker({dateFormat: 'dd-mm-yy', dayNamesMin: ['D', 'L', 'M', 'M', 'J', 'V', 'S'], firstDay: 1, monthNames:['Ianuarie', 'Februarie', 'Martie', 'Aprilie', 'Mai', 'Iunie', 'Iulie', 'August', 'Septembrie', 'Octombrie', 'Noiembrie', 'Decembrie']});
				jQuery('.wp-crm-form-matrix-add-row', txt).click(function(e){
					e.preventDefault();
				...
				jQuery('.wp-crm-form-matrix-add-col', txt).click(function(e){
					e.preventDefault();
					var k = 0;
				...
				jQuery('.wp-crm-form-filedrop', txt).each(function(i,u){
					u = jQuery(u);
					u.filedrop({
			...
			if ($wpcrmui.txt.length > 1) {
				$wpcrmui.txt.last().hide().remove();
				$wpcrmui.txt.last().show();
				}
			else {
